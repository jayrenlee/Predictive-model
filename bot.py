...
PENANG_ASC_DEG  = 100.3   # Geodetic ASC = longitude
PENANG_KEY_PTS  = [100.3, 190.3, 280.3, 10.3]  # ASC, IC, DSC, MC
...
def get_geodetic_info(today: date) -> dict:
    """
    ...
    sun_house  = int((sun_lon  - PENANG_ASC_DEG) % 360 // 30) + 1
    moon_house = int((moon_lon - PENANG_ASC_DEG) % 360 // 30) + 1

    # Aspect detection
    active_aspects = []
    angle_names    = ["ASC","IC","DSC","MC"]
    for pt, ang_name in zip(PENANG_KEY_PTS, angle_names):
    ...
    natal_digits = GEO_SIGN_DIGITS["Cancer"]
...
def get_jackpot_pairs(maps: dict, n: int = JACKPOT_N) -> list:
    ...
    preds = get_predictions(maps, n=88)
    if not preds:
        return []
...
def handle(message: dict, data: dict):
    ...
    if lower.startswith("/lucky"):
        ...
        for num, model_score, stars, matched in scored:
            bar      = "█" * round(model_score * 10) + "░" * (10 - round(model_score * 10))
            star_str = "⭐" * stars + "·" * (2 - stars)
            m_str    = "+".join(matched) if matched else "no match"
            lines.append(f"`{num}`  {star_str}  {bar}  _{m_str}_")

        lines.append(f"\n_Prediction model + geodetic astrology + I-Ching_")
