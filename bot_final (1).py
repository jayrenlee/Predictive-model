...
WISMA_ASC_DEG   = 101.7136  # Geodetic ASC = longitude (Wisma Magnum, Jalan Pudu, KL)
WISMA_KEY_PTS   = [101.7136, 191.7136, 281.7136, 11.7136]  # ASC, IC, DSC, MC
...
def get_geodetic_info(today: date) -> dict:
    ...
    sun_house  = int((sun_lon  - WISMA_ASC_DEG) % 360 // 30) + 1
    moon_house = int((moon_lon - WISMA_ASC_DEG) % 360 // 30) + 1

    # Aspect detection
    active_aspects = []
    angle_names    = ["ASC","IC","DSC","MC"]
    for pt, ang_name in zip(WISMA_KEY_PTS, angle_names):
...
def handle(message: dict, data: dict):
    ...
    if lower.startswith("/geo"):
        ...
        lines = [
            ...
            f"*Wisma Magnum, Jalan Pudu, KL (101.7°E)*\n",
    ...
    if lower.startswith("/lucky"):
        ...
        lines = [
            ...
            f"🌍 *Geodetic (Wisma Magnum, KL 101.7°E):*",
            f"   ☀️ Sun {geo['sun_deg']}° {geo['sun_sign']} {geo['sun_sym']} "
            f"(H{geo['sun_house']}) → *{', '.join(str(d) for d in geo['sun_digits'])}*",
            f"   🌙 Moon {geo['moon_deg']}° {geo['moon_sign']} {geo['moon_sym']} "
            f"(H{geo['moon_house']}) → *{', '.join(str(d) for d in geo['moon_digits'])}*",
